...
const CODEX_ADAPTER_SOURCE = "codex";
/**
 ...
 */
const CODEX_HOOK_LIFECYCLES = Object.freeze([
    SESSION_STATUS.STARTED,
    SESSION_STATUS.COMPLETED,
    SESSION_STATUS.PANE_DISAPPEARED,
]);
function parseCodexHookPayload(raw) {
    let candidate;
    try {
    ...
    if (typeof candidate !== "object" || candidate === null || Array.isArray(candidate))
        return undefined;
    const { session_id: sessionId } = candidate;
    if (typeof sessionId !== "string" || sessionId.length === 0)
        return undefined;
    return { sessionId };
}
function codexLifecycleFor(lifecycle) {
 ...
 * always succeeds.
 */
const CODEX_HOOK_EXIT_CODE = 0;
const STDIN_BYTE_LIMIT = 64 * 1024;
const LIFECYCLE_FLAG = "--lifecycle";
function buildCodexHookArgv(sessionId, lifecycle, environment) {
...
function createCodexHookEmit(environment) {
...
if (isDirectCliInvocation(import.meta.url, process.argv[1])) {
    ...
    readHookInput()
        ...
        .then(() => { process.exitCode = CODEX_HOOK_EXIT_CODE; }, () => { process.exitCode = CODEX_HOOK_EXIT_CODE; });
}

export { CODEX_HOOK_EXIT_CODE, buildCodexHookArgv, createCodexHookEmit, runCodexHook };
