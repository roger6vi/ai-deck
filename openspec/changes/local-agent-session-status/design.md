...
Reducer rules: first-free assignment; sixth session remains unassigned/capacity-full; slots stay stable while session and pane exist; duplicate `eventId` and stale `(timestamp, sequence)` are ignored; colors are green free/idle/read, amber running under five minutes, red error or running at/over five minutes, blue completed unread. Five-minute red is advisory only. Visibility/manual navigation never acknowledges; only Stream Deck `onKeyDown` for that assigned action can acknowledge. Missing pane releases immediately.
